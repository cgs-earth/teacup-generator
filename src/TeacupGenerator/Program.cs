...
namespace Teacup
{
    ...
    {    
        ...
        public static void Main(string[] args)
        {
            ...
            DateTime tStart = DateTime.Parse(args[0].ToString());
            DateTime tEnd = DateTime.Parse(args[1].ToString());
        ...
        private static string FindDroughtBaseMap(DateTime t)
        {
            ...
            //Console.WriteLine(t.ToShortDateString() + " | " + "DroughtBaseMap_" + tuesdayT.ToString("yyyyMMdd") + ".png" + "|" + fileExistsBool);
        ...
        private static Bitmap BuildHistoricalCharts(string[] lines, DateTime t)
        {
            ...
            Bitmap bmp;
            //@"\\ibr8drogis02.bor.doi.net\DroughtBaseMaps_png\";
            //var imgURL = @"file://ibr8drogis02.bor.doi.net/DroughtBaseMaps_png/" + baseMapName;
            ...
            // Get DataFile 
            //var dataURL = @"file://ibr8drogis02.bor.doi.net/DroughtBaseMaps_png/data/droughtData" + t.ToString("yyyyMMdd") + ".csv";
            var dataURL = @"datafiles/droughtData" + t.ToString("yyyyMMdd") + ".csv";
            if (t.Month == 2 && t.Day == 29)
            {
                //dataURL = @"file://ibr8drogis02.bor.doi.net/DroughtBaseMaps_png/data/droughtData" + t.AddDays(-1).ToString("yyyyMMdd") + ".csv";
        ...
        private static void generateStandAloneHistoricalTeacupImage(ConfigLine cfg, DateTime t)
        {
            ...
            cfg.row = 90;
            cfg.col = 80;
            //var fName = @"\\ibr8drogis02.bor.doi.net\DroughtBaseMaps_png\teacups\"+ cfg.DisplayName.Replace(" ", "") + t.ToString("yyyyMMdd") + ".png";
            //var fName = @"\\ibr8drofp001\PROD_RISEDATA\rise_resviz\intake\teacups\" + cfg.DisplayName.Replace(" ", "") + t.ToString("yyyyMMdd") + ".png";
        ...
        private static void GenerateStaticMapFile()
        {
            DateTime nowPST = TimeZoneInfo.ConvertTimeFromUtc(
                    DateTime.UtcNow, TimeZoneInfo.FindSystemTimeZoneById("Pacific Standard Time"));
            string mapDate = nowPST.AddDays(-1).ToString("yyyyMMdd");
            if (nowPST.Hour <= 7)
            {
                mapDate = nowPST.AddDays(-2).ToString("yyyyMMdd");
            }
            string mapPath = @"teacups\usbrTeacups" + mapDate + ".pdf";
            try
            {
                File.Copy(mapPath, @"teacups\USBR_Tea_Cup_Current.pdf");
            }
            catch
            {
                Console.WriteLine("Could not generate a static map file, map not found");
            }
        }
        ...
        private static Bitmap BuildTeacupCharts(string[] lines)
        {
            ...
            if (useOnlineMapFile) 
            {
                var imgURL = @"https://ibr2mprgis005.bor.doi.net/WebApps/DroughtBaseMap/DroughtBaseMap.png";
                WebClient client = new WebClient();
                Stream stream = client.OpenRead(imgURL);
                var gif = new Bitmap(stream);
                stream.Flush();
            ...
            { //from USBR public web
                var dataURL = "https://www.usbr.gov/lc/region/g4000/riverops/webreports/dailyDroughtReservoirData.csv";
                mainDataFile = (new WebClient()).DownloadString(dataURL);
            }
            else
            { //from same directory
